...
const mutations: MutationTree<ChatState> = {
  // 保存socket
  [SET_SOCKET](state: { socket: SocketIOClient.Socket }, payload: SocketIOClient.Socket) {
    state.socket = payload
  },

  // 设置用户是否处于掉线重连状态
  [SET_DROPPED](state: { dropped: boolean }, payload: boolean) {
   ...
   */
  [USER_ONLINE](
    state: { friendGather: { [x: string]: any }; groupGather: { [s: string]: Group } | ArrayLike<Group> },
    userId: string
  ) {
    // 更新好友列表用户状态
  ...
  // 新增群成员
  [ADD_GROUP_MEMBER](
    state: { groupGather: { [x: string]: any } },
    payload: {
      groupId: string
      members: Friend[]
    }
  ...
  // 新增一条群消息
  [ADD_GROUP_MESSAGE](state: { groupGather: { [x: string]: any } }, payload: GroupMessage) {
  ...
  // 设置群消息
  [SET_GROUP_MESSAGES](state: { groupGather: { [x: string]: any } }, payload: GroupMessage[]) {
  ...
  // 新增一条私聊消息
  [ADD_FRIEND_MESSAGE](state: { friendGather: { [x: string]: any } }, payload: FriendMessage) {
    // @ts-ignore
  ...
  // 设置私聊记录
  [SET_FRIEND_MESSAGES](state: { friendGather: { [x: string]: any } }, payload: FriendMessage[]) {
    // @ts-ignore
    const { userId } = this.getters['app/user']
    if (payload && payload.length) {
  ...
  // 设置当前聊天对象(群或好友)
  [SET_ACTIVE_ROOM](state: any, payload: Friend & Group) {
    state.activeRoom = payload
  },

  // 设置所有的群的群详细信息(头像,群名字等)
  [SET_GROUP_GATHER](state: { groupGather: any }, payload: Group) {
    Vue.set(state.groupGather, payload.groupId, payload)
  },

  // 设置所有的用户的用户详细信息(头像,昵称等)
  [SET_USER_GATHER](state: { userGather: any }, payload: User) {
    Vue.set(state.userGather, payload.userId, payload)
  },

  // 设置所有的好友的用户详细信息(头像,昵称等)
  [SET_FRIEND_GATHER](state: { friendGather: any }, payload: User) {
    Vue.set(state.friendGather, payload.userId, payload)
  },

  // 设置所有的用户的用户详细信息(头像,昵称等)
  [UPDATE_USER_INFO](state: { userGather: any; friendGather: any }, user: User) {
  ...
  // 退群
  [DEL_GROUP](state: any, payload: GroupMap) {
    Vue.delete(state.groupGather, payload.groupId)
  },

  // 删除群成员
  [DEL_GROUP_MEMBER](state: any, payload: GroupMap) {
    const group = state.groupGather[payload.groupId]
    if (group) {
      group.members = group.members!.filter((member: any) => member.userId !== payload.userId)
    }
  },

  // 删好友
  [DEL_FRIEND](state: any, payload: UserMap) {
    Vue.delete(state.friendGather, payload.friendId)
  },

  // 给某个聊天组添加未读消息
  [ADD_UNREAD_GATHER](state: any, payload: string) {
  ...
  // 给某个聊天组清空未读消息
  [LOSE_UNREAD_GATHER](state: any, payload: string) {
    document.title = 'TylooChat聊天室'
    Vue.set(state.unReadGather, payload, 0)
  },

  // 消息撤回
  [REVOKE_MESSAGE](state: any, payload: FriendMessage & GroupMessage & { username: string }) {
    // @ts-ignore
    const { userId } = this.getters['app/user']
    // 撤回的为群消息
    if (payload.groupId) {
      ...
      if (messages) {
        const msg = messages.find((message: any) => message._id === payload._id)
        if (msg) {
